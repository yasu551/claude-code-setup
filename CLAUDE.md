...
├── wizard.ts       # Adaptive question flow for interactive /init
...
├── init.ts         # /init orchestrator (URL fetch or wizard generation)
...
- **Dual-source init**: `/init` supports both fetched profiles (URL argument) and generated profiles (interactive wizard). Both paths share the same merge engine and lockfile system.
